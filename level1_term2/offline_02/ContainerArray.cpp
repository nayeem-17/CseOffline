...
public:
    ...
    void setItemAt(Container con, int index)
    {
        if (index >= allocatedSize)
            cout << "Exception: Container Array index out of bound";
        int tp = con.getStoredType();
        int *temp, **temp1;
        if (tp != 3)
            temp = (int *)con.getItem();
        else
            temp1 = (int **)con.getItem();
        if (tp == 1)
            arrayOfContainers[index].setItem(*temp);
        if (tp == 2)
            arrayOfContainers[index].setItem(temp, con.getFirstDim());
        if (tp == 3)
            arrayOfContainers[index].setItem(temp1, con.getFirstDim(), con.getSecondDim());
    }
...
int main()
{
    ...
    {
        Container a;
        //    cout<<"pleas print ! pleas print ! pleas print ! pleas print !pleas print ! pleas print !pleas print ! pleas print !"<<endl;
